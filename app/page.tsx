...
const features = [
  ...
  },
  {
    ...
    desc: "Upload a spreadsheet with hundreds of employees — get all salaries predicted and download a results CSV.",
    href: "/bulk",
  },
];
          ...
          {features.map((f) => {
            const inner = (
              <>
                ...
                {"href" in f && (
                  <span className="mt-3 inline-flex items-center gap-1 text-xs text-teal-400">
                    Try it
            ...
            return "href" in f ? (
              <Link
                key={f.title}
                href={f.href!}
                className="card-glass rounded-2xl p-6 hover:border-teal-500/50 transition-all duration-300 block"
              >
                {inner}
              </Link>
            ) : (
